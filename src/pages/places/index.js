...
const typeref = {
  ...
  const nearbySearch = async (mapInstance, type) => {
    ...
    const request = {
      fields: ['name', 'geometry', 'vicinity', 'photos'], // Request photos as well
      location: center,
      radius: 50000,
      type: [type],
    };
      ...
      {places.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {places.map((place) => {
            ...
            return (
              <div key={place.place_id} className="relative card p-4 bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden">
                <a
                ...
                >{photoUrl && (
                  <img src={photoUrl} alt={place.name} className="w-full h-40 object-cover mb-2" />
                )}
                <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-60 text-white opacity-0 hover:opacity-100 transition-opacity duration-300">
