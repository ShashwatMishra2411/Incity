...
function Commute() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          setChat((prevChat) => [
            ...prevChat,
            { type: "bot", text: "Unable to retrieve your location" },
          ]);
        }
      );
    } else {
      setChat((prevChat) => [
        ...prevChat,
        { type: "bot", text: "Geolocation is not supported by this browser." },
      ]);
    }
  };
  ...
  const handleSend = async () => {
    if (!destination) {
      ...
      try {
        ...
        setChat((prevChat) => [
          ...prevChat,
          { type: "bot", text: "Error fetching destination coordinates" },
        ]);
      }
    } else {
    ...
    if (mode === "Public") {
      if (currentLocation && destinationLat && destinationLng) {
        ...
        setChat((prevChat) => [
          ...
          {
            type: "bot",
            text: "Please get your current location and set a valid destination first.",
          },
        ]);
      }
    }
  ...
  const getPublicTransport = async () => {
    try {
        ...
        { type: "bot", text: geminiResponseText },
        {
          ...
          text: (
            <>
              ...
              <a
                href={`https://www.google.com/maps/dir/?api=1&origin=${currentLocation.latitude},${currentLocation.longitude}&destination=${destinationLat},${destinationLng}`}
                target="_blank"
      ...
      setChat((prevChat) => [
        ...prevChat,
        { type: "bot", text: "Error fetching transit data" },
      ]);
    }
  };
